...
function createJourneyMarkup(data) {
  ...
  const milestoneHtml = REWARD_LADDER.map((item) => {
    const reached = streak >= item.days;
    const next = item.days === nextMilestone && !reached;
    const classes = [
      "jt-streak-milestone",
      reached ? "reached" : "",
      next ? "next" : ""
    ...
    return `
      <div
        class="${classes}"
        title="${item.label}: +${item.points} performance points"
      >
        <b>${item.days}D</b>
  ...
  return `
    ...
    <div
      class="jt-streak-milestones"
      aria-label="Streak reward milestones"
    >
      ${milestoneHtml}
    </div>
